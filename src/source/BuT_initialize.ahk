...
rev=%Revision%
name=BuTPUQ r%Revision%-r%CompRevision% (%A_ScriptName%)
...
; Portable NeoVars Instanz?
if (FileExist(A_AppData . "\Neo2\Neo2.ini")) {
  ApplicationFolder := A_AppData . "\Neo2"
} else {
  ApplicationFolder := A_ScriptDir
}

FileCreateDir, %ApplicationFolder%
ini := ApplicationFolder . "\Neo2.ini"

IniRead,zeigeLockBox,%ini%,Global,zeigeLockBox,0
