...
AllStar(This_HotKey) {
  ...
  if (char == "U0000F6") && (nch == 8)                 ; 30key custom layout exception
    Char := "T__diak"
  if (char == "U0000D6") && (nch == 8) 
    Char := "S__comp"
    
     if (SubStr(Char,4,1) == "N") && (Isdown = 1)      ; create var for compose 
  or if (SubStr(Char,1,1) == "U") && (Isdown = 1) {    ; and diacritica    
    pchar3 := pchar2
    pchar2 := pchar1
    pchar1 := Char
    }
  
  if (isLayout > 5)                                    ; var for layout switch
       30keys := 1
  else 30keys := 0
  
  if (isLang != "")                         
      Lang := %islang%                                 ; diacritica language  
      oldlang := islang  
      DLang(lang)
  
  if (char = "T__diak") {                              ; diacritica key increment   
      diak := ++diak
      scomp := 1                                       ; var for compose exception
 } if (char != "T__diak") 
      diak := 0
 
  if (diak > 0) && (isdown = 1)                        ; call diacritica function
      Diakritika(lang,diak)
  if (diak = 1) && (isdown = 0)                        ; sloppy typing corrective
      diak := ++diak
  if  diak > %Maxind%                                  ; diacritica cycle limiter
      diak := 0

 if (char = "S__comp")                                 ; call compose function
      Composite(char,pchar1,pchar2,pchar3)
...
rerun:
  ...
  if (diak = 0) 
    ...
    CompNew := Comp . pchar1               ; Diacritica reverse
    
  if (CD%CompNew% != "") {                 ; Compose hat getroffen: wird geschickt, Compose gelöscht

 
  if (char = "T__diak") && (Diak > 0)      ; Diacritica Backspacing
    send {BS}
 
  tosend := CD%CompNew%
    PP%PhysKey% :=  CompNew
    Comp := ""
  } else if (CM%CompNew% == 1) {           ; Compose muss sich noch was merken: 
    tosend := ""                           ; Jetzt noch nichts schicken.
    PP%PhysKey% := ""
    Comp := CompNew
  } else if (CF%Comp% != "") {
    tosend := CF%Comp%
