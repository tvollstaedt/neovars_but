...
set fnexe=%outdir%\BuT.exe
"%SubWCRev%" "%svnversiondir1%" -nm
if errorlevel 1 (
  set fnexe=%outdir%\BuT-r%Revision%.exe
)

echo removing old version(s) of NEO AHK Exe file
del "%outdir%\BuT-r*.exe" 2> nul

set fnahk=%srcdir%\BuT_all.ahk
...
"%Ahk2Exe%" /in "%fnahk%" /out "%fnexe%" /icon "%srcdir%\BuT_enabled.ico"
